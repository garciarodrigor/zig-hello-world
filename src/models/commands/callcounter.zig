...
pub const CallCounterCommand = struct {
    const Self = @This();
...
test "models/commands/CallCounterCommand.init" {
    const obj = CallCounterCommand.init(SingleCommand.init("test-name").command());

    try testing.expectEqual(@as([]const u8, "test-name"), obj.getName());
}

test "models/commands/CallCounterCommand.execute" {
    var obj = CallCounterCommand.init(SingleCommand.init("test-name").command());
